...
- 每个字符生成一个 `.litematic` 文件，全部文件再装入一个 TAR 下载。
...
5. 点击“导出全部投影”。
6. 浏览器先生成临时 PNG，随后本机服务将其转换为 `.litematic`，最终下载 `字符投影_日期_时间.tar`。
...
2. 调整样式并点击“导出全部投影”。
3. 等待页面经历“生成画布”“本地服务正在转换投影”“正在下载投影 TAR”三个阶段。
...
Windows 也可以使用 7-Zip 打开 TAR。确认压缩包中只有 `.litematic` 文件，并且不存在大小写文件名冲突。
